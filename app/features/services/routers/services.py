...
from app.features.services.schemas import ServiceResponse, ServiceAvailabilityResponse
...
@router.get("/available/regions/{region_id}", response_model=BaseAPIResponse[PaginatedData[ServiceResponse]], status_code=status.HTTP_200_OK)
async def get_available_services_in_region(
    ...
    try:
        cache_key = f"services::available:region:{region_id}:page:{page}:per_page:{per_page}"
        ...
        statement, count_statement = ServicesQueries.get_available_services_in_region_query(region_id)
        ...
        await cache_service.set_json(cache_key, data.model_dump(mode='json'), expire=300)
...
@router.get("/{service_id}/available/regions/{region_id}", response_model=BaseAPIResponse[ServiceAvailabilityResponse], status_code=status.HTTP_200_OK)
async def check_service_availability_in_region(
    ...
    try:
        ...
        statement = ServicesQueries.check_service_availability_in_region_query(service_id, region_id)
...
@router.get("/{service_id}/providers/regions/{region_id}", response_model=BaseAPIResponse[PaginatedData[UserResponse]], status_code=status.HTTP_200_OK)
async def get_providers_for_service_in_region(
    ...
    try:
        ...
        if cached_data:
            return BaseAPIResponse[PaginatedData[UserResponse]](
                data=PaginatedData[UserResponse](**cached_data),
                detail="Providers retrieved successfully from cache.",
                status_code=status.HTTP_200_OK
            )
            
        statement, count_statement = ServicesQueries.get_providers_for_service_in_region_query(service_id, region_id)
