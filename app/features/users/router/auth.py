...
async def register(
    ...
    try:
        ...
        await system_logger.metric(f"User registration: {schema.email}", timer.stop(), source="auth.register")
...
async def login(
    ...
    try:
        ...
        await system_logger.metric(f"User login: {form_data.username}", timer.stop(), source="auth.login")
