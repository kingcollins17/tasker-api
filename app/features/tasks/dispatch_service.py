...
from app.features.tasks.celery.dispatch import execute_matching_engine
...
class DispatchService:
    ...
    async def _save(self, entity: Any) -> Any:
        """Adds entity to database session, commits, and refreshes."""
    ...
    async def _log_task_event(
        ...
        **kwargs,
        ...
        return await self._save(history)
    ...
    async def _get_next_sequence(self, task_id: str) -> int:
        ...
        res = await self.session.exec(stmt_max)
    ...
    async def manual_redispatch(
        ...
        if task.status == TaskStatus.ASSIGNED and task.assignment:
            ...
            await self._save(task.assignment)
    ...
    async def handle_no_match(
        ...
        dispatch_session = await self.session.get(DispatchSession, session_id)
        ...
        if dispatch_session:
            ...
            await self._save(dispatch_session)
    ...
    async def _handle_auto_exhaustion(
        ...
        task.status = TaskStatus.NO_MATCH
    ...
    async def handle_ping_response(
        ...
        res_attempt = await self.session.exec(stmt_attempt)
        ...
        if new_status == DispatchAttemptStatus.ACCEPTED:
            ...
            res_existing_accept = await self.session.exec(stmt_existing_accept)
    ...
    async def _process_acceptance(
        ...
        res_assign = await self.session.exec(stmt_assign)
        ...
        if assignment:
            ...
            await self._save(new_assignment)
        ...
        res_prof = await self.session.exec(stmt_prof)
        ...
        await self.session.exec(stmt_session)
        ...
        await self.session.exec(stmt_cancel)
    ...
    async def _process_decline_or_timeout(
        ...
        await self.credibility_service.add(
            user_id=provider_id,
            reason=CredibilityReason.TASK_DECLINED,
            task_id=task_id,
        )
        ...
        res_pending = await self.session.exec(stmt_pending)
